...
def create_sql_dump(db_file, dump_file, drop_table=True, export_mode="both"):
    ...
    for idx, table in enumerate(tables, 1):
        ...
        for col_info in columns_info:
            ...
            if col_type == 'TEXT':
                cursor.execute(f"SELECT MAX(LENGTH({col_name})) FROM {table};")
                max_length = cursor.fetchone()[0]
                max_lengths[col_name] = max(max_lengths.get(col_name, 0), max_length)
        ...
        with open(dump_file, 'a') as f:
            ...
            if export_mode in ("data", "both"):
                ...
                for row_num, row in enumerate(data, 1):
                    ...
                    row = [col if col is not None and col != '' else 'NULL' for col in row]
                    ...
                    columns = ', '.join('NULL' if col is None or col == '' else repr(col) for col in row)
                    ...
                    print(f"Progress: {row_num}/{total_rows} rows exported.", end='\r')
...
def display_help():
    print("Usage: python sqlite_to_mysql.py <sqlite_db_file> [mysql_dump_file] [--no-drop]")
    print("Parameters:")
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="SQLite to MySQL SQL dump converter")
    parser.add_argument("sqlite_db_file", type=str, help="Path to the SQLite database file.")
    ...
    parser.add_argument("--no-drop", action="store_true", help="Prevents adding 'DROP TABLE IF EXISTS' statement in the SQL dump.")
    parser.add_argument("--export-mode", choices=["structure", "data"], help="Choose export mode: structure or data.")
...
if __name__ == '__main__':
    ...
    mysql_dump_file = args.mysql_dump_file if args.mysql_dump_file else os.path.splitext(sqlite_db_file)[0] + '.sql'
    ...
    if export_mode == "structure":
        create_sql_dump(sqlite_db_file, mysql_dump_file, drop_table, export_mode)
